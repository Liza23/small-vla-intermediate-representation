"""
Training script for VLA v3.0 with Future Prediction
...
from models import VLAModelV3
from data.libero_dataset_v3 import create_dataloaders_v3
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        future_images = batch['image_next']
        ee_pose_next = batch['ee_pose_next']  # From V2: t+1 ee_pose.to(device)  # NEW: t+1 images
        ...
        outputs = model(
            ...
            future_ee_pose=ee_pose_next,  # NEW: GT future for loss
        )
        ...
        if rank == 0:
            pbar.set_postfix({
                'loss': loss.item(),
                'img': loss_future.item(),
                'ee': loss_ee_pose.item(),
                'act': loss_action.item(),
            })
    ...
    return avg_loss, avg_loss_future, avg_loss_action, avg_loss_decoder, avg_loss_ee_pose, global_step
...
def validate(
    ...
    return avg_loss, avg_loss_future, avg_loss_action, avg_loss_decoder, avg_loss_ee_pose
...
def main():
    parser = argparse.ArgumentParser(description='Train VLA v3.0 model with future prediction')
    ...
    if rank == 0:
        print(f"Training VLA v3.0 with {world_size} GPUs")
    ...
    output_dir = os.path.join(
        ...
        f"{config['data']['task_name']}_v3_{timestamp}"  # v1 suffix
    )
    ...
    if world_size > 1:
        ...
        from data.libero_dataset_v3 import LIBERODatasetV3, collate_fn_v3
        from torch.utils.data import DataLoader

        dataset = LIBERODatasetV3(
    ...
    model = VLAModelV3(
        ...
        future_loss_weight=config['training'].get('future_loss_weight', 0.5),
    ...
    if rank == 0:
        print("\n" + "="*50)
        print("Starting VLA v3.0 training")
        print("="*50)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        # Train
        train_loss, train_loss_future, train_loss_action, train_loss_decoder, train_loss_ee, global_step = train_epoch(
            model=model,
        ...
        if val_loader is not None and rank == 0 and len(val_loader) > 0:
            val_loss, val_loss_future, val_loss_action, val_loss_decoder, val_loss_ee = validate(
                model=model,
        ...
        # Log
        if rank == 0:
            print(f"Epoch {epoch}: train={train_loss:.4f} (future={train_loss_future:.4f}, ee={train_loss_ee:.4f}, action={train_loss_action:.4f}), val={val_loss:.4f}")
            ...
            if writer is not None:
                ...
                writer.add_scalar('epoch/val_loss_ee_pose', val_loss_ee, epoch)
